...
const DB = (() => {
  ...
  async function init() {
    return new Promise((resolve, reject) => {
      ...
      req.onupgradeneeded = (e) => {
        ...
        if (old < 3) {
          // item_stocks: qty → count
          const stockStore = t.objectStore('item_stocks');
          stockStore.getAll().onsuccess = (ev) => {
            for (const s of (ev.target.result || [])) {
              s.count = parseFloat(s.qty) || 0;
              delete s.qty;
              stockStore.put(s);
            }
          };
          // items: unit → count_unit、volume / volume_unit を追加
          if (old >= 1) {
            const itemStore = t.objectStore('items');
            itemStore.getAll().onsuccess = (ev) => {
              for (const item of (ev.target.result || [])) {
                item.count_unit  = item.unit || '個';
                item.volume      = null;
  ...
  const ItemStocks = {
    ...
    async replaceForItem(itemId, entries) {
      ...
      for (const e of entries) {
        if ((parseFloat(e.count) || 0) > 0) {
          await add('item_stocks', {
            item_id:     itemId,
            count:       parseFloat(e.count) || 0,
    ...
    deleteForItem: async (id) => {
      const all = await getByIdx('item_stocks', 'item_id', id);
      for (const s of all) await del('item_stocks', s.id);
    },
  };
  ...
  function enrichItems(items, stockMap) {
    return items.map(item => {
      ...
      const total_count  = stocks.reduce((s, e) => s + (parseFloat(e.count) || 0), 0);
      const future       = stocks.filter(s => s.expiry_date && Utils.daysUntil(s.expiry_date) >= 0);
      const nearest_expiry = future.length
        ? future[0].expiry_date
        : (stocks.find(s => s.expiry_date)?.expiry_date || '');
      return {
        ...item,
        stocks,
        total_count,
        total_qty: total_count, // 後方互換エイリアス（dashboard.js 等向け）
        nearest_expiry,
      };
    });
  }
  ...
  const ItemImages = {
    ...
    deleteForItem: async (id) => {
      const imgs = await getByIdx('item_images', 'item_id', id);
  ...
  const Shopping = {
    ...
    clearBought: async () => {
      ...
      for (const s of all) if (s.is_bought) await del('shopping_list', s.id);
    },
  };

  const Settings = {
    get: async (key, def = null) => {
      try { const r = await get('settings', key); return r ? r.value : def; } catch { return def; }
    },
